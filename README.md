...
- **`publish_web_layer` can fail with a generic `ArcGIS ERROR 999999`**, even for data that publishes fine seconds later through ArcGIS Pro's own Share As Web Layer UI on the same portal and account. Live-confirmed: passing the map object vs. a single layer made no difference, and layer-ID assignment was ruled out — the most likely cause is this bridge's background-thread execution model (`StageService` appears to need the signed-in portal session, which isn't usable from a background thread the way read-only calls like `GetActivePortalURL()` are). If you hit this, publish through the Pro UI directly instead of retrying — this isn't fixable without restructuring the bridge to marshal specific calls onto the main thread, which is a bigger change than anything else here.
